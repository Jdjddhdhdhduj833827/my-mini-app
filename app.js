/* app.js — ТОТ ЖЕ ИНТЕРФЕЙС, НО БЕЗ ЛЮБЫХ ПРОВЕРОК РЕГ/ДЕПОЗИТА
   Любой пользователь может зайти и нажимать "Запустить анализ".
   Ссылки REG_URL / DEPOSIT_URL остаются и работают.
*/
...
(() => {
  ...
  // ---------- State ----------
  let LANG = "ru";
  let NOTIFY_MODE = "deposit"; // "reg" | "deposit"

  // ВАЖНО: больше НЕ используем серверную проверку вообще
  const AUTH = {
    ok: true,
    telegram_id: "",
    access: true,
    vip: false,
    flags: { registered: 1, dep_count: 1, approved: 0 },
  };
  ...
  function premiumNotify(mode, title, text){
    ...
    show(notify);
    setLocked(false); // НИКАКОЙ блокировки больше
  }

  function setLocked(isLocked){
    // Оставляем поддержку, но всегда отключаем лок
    if (!softLock) return;
    if (isLocked) show(softLock);
    else hide(softLock);

    if (btnAnalyze) btnAnalyze.disabled = isLocked;
    if (btnLong) btnLong.disabled = isLocked;
    if (btnShort) btnShort.disabled = isLocked;

    if (btnAnalyze) btnAnalyze.style.opacity = isLocked ? ".55" : "";
    if (btnLong) btnLong.style.opacity = isLocked ? ".55" : "";
    if (btnShort) btnShort.style.opacity = isLocked ? ".55" : "";
  }

  // ---------- Gate flow (БЕЗ ПРОВЕРОК) ----------
  function gateProceedNoChecks(){
  ...
  function enterApp(){
    ...
    // session/access purely local now
    const suffix = tg?.initDataUnsafe?.user?.id ? String(tg.initDataUnsafe.user.id).slice(-6) : "LOCAL";
    if (chipSession) chipSession.textContent = "SESSION: " + suffix;
    if (chipAccess) chipAccess.textContent = "ACCESS: OPEN";
  ...
  function openLangModal(){
    ...
    items.forEach(it => {
      ...
      row.addEventListener("click", () => {
        ...
        applyI18n();
        if (notify && !notify.classList.contains("hidden") && notifyPrimaryLabel) {
  ...
  async function runAnalysis(direction /* "long" | "short" */){
    ...
    if (chartOverlay) hide(chartOverlay);
    ...
    if (rUntil) rUntil.textContent = `${String(until.getHours()).padStart(2,"0")}:${String(until.getMinutes()).padStart(2,"0")}`;
  ...
  function setSignal(text, isUp){
    ...
    if (isUp) {
      if (dirArrow) dirArrow.textContent = "↗";
  ...
  btnGetAccess?.addEventListener("click", gateProceedNoChecks);
  ...
  // "Проверить" теперь просто показывает, что всё доступно (без запросов)
  btnCheckStatus?.addEventListener("click", () => {
    // можно вообще ничего не делать, но оставим мягкую подсказку:
    premiumNotify("deposit", "STATUS", "Доступ открыт. Проверки регистрации/депозита отключены.");
    // и сразу можно закрыть, если не хочешь окна:
    // hide(notify);
  });
  ...
  (async function boot(){
    ...
    // ВАЖНО: теперь ничего не проверяем — просто рендерим демо на gate
    drawChartDemo();
    drawMiniChartDemo();

    // На всякий случай: softLock всегда выключен
    setLocked(false);
  })();
})();
